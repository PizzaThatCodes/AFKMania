...
public class AFKBlockModules implements ModuleInterface {
    ...
    private void setupPlayerDataStore() {
        playerDataFile = new File(Main.getInstance().getDataFolder(), "modules/afk_block_data.yml");
        if (!playerDataFile.exists()) {
            try {
                if (playerDataFile.getParentFile() != null && !playerDataFile.getParentFile().exists()) {
                    playerDataFile.getParentFile().mkdirs();
                }
                playerDataFile.createNewFile();
            } catch (IOException e) {
                Main.getInstance().getLogger().severe("Failed to create afk_block_data.yml");
                e.printStackTrace();
            }
        }
    ...
    private void loadUpgrades(Player player, AFKBlockPlayerData data) {
        ConfigurationSection section = playerDataConfig.getConfigurationSection("players." + player.getUniqueId() + ".upgrades");
    ...
    public void saveUpgrades(AFKBlockPlayerData data) {
        String basePath = "players." + data.getPlayer().getUniqueId() + ".upgrades";
        for (Map.Entry<String, Integer> entry : data.getUpgradeLevels().entrySet()) {
            playerDataConfig.set(basePath + "." + entry.getKey(), entry.getValue());
        }
        try {
            playerDataConfig.save(playerDataFile);
        } catch (IOException e) {
            Main.getInstance().getLogger().severe("Failed to save AFK block player data");
            e.printStackTrace();
        }
    }
